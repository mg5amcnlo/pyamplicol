...
- When the source revision changes, keep strict same-source reuse as the default and retain earlier currents as history without content-digest comparisons. Offer explicit `run --continue-across-revisions` continuation: structurally valid completed cells explicitly selected by the user may be recycled from older revisions, while missing cells and every dependency needed by newly run work must use the active clean source revision.
- Persist the explicit continuation policy in the lightweight source marker so `inspect` and `refresh-pdf` automatically include the complete mixed per-cell current set and prominently report every exact source revision and cell count. Each cell retains its own source provenance; never present a mixed report as one source cohort.
- Reuse valid same-source results by default. `--force-refresh` creates a new regeneration and remeasurement attempt even for a historical current accepted by continuation, without deleting history. Replace current only with a completed success or a resource-cap terminal result.
...
- Refuse real measurements unless the installed runtime has the exact clean committed source revision selected by the controller. Never weaken recurrence runtime identity checks or combine evidence from different revisions inside one cell; an explicitly continued report may contain separate per-cell results from several clearly reported source cohorts.
