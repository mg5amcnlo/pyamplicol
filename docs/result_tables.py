...
def render_model_ladder(spec: LadderSpec, payload: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        r"\begin{center}",
        r"\begin{tabular}{@{}cL{2.95in}ccccc@{}}",
        r"\toprule",
        (
            r"$n$ & \textbf{process} & \textbf{status} & "
            r"\textbf{gen. [s]} & \textbf{wall [s/pt]} & "
            r"\textbf{value} & \textbf{rel. diff.} \\"
        ),
        r"\midrule",
    ]
    for n_final in spec.multiplicities:
        entry = entries[n_final]
        measurement = entry["measurement"]
        assert isinstance(measurement, Mapping)
        process = _tex_escape(str(entry["process"]))
        lines.append(
            f"{n_final} & \\texttt{{{process}}} & \\ReportNA & "
            f"{_format_number(measurement['generation_seconds'])} & "
            f"{_format_number(measurement['wall_seconds_per_point'])} & "
            f"{_format_number(measurement['matrix_element'])} & "
            f"{_format_number(entry['relative_difference'])} \\\\"
        )
    lines.extend(
        [
            r"\bottomrule",
