...
On a healthy published 0.1.3 macOS arm64 installation, the colored terminal
tables report checks equivalent to this compact transcription:

```text
python          pass
model-assets    pass   67 verified files
rusticol-python pass   package 0.1.3, C ABI 1
...
## Recurrence or eager says a prepared model is missing

Recurrence and eager use prepared local kernels. A raw UFO directory, serialized
model JSON, or compiled model IR is not itself a prepared kernel bundle.
