...
```toml
[evaluator]
execution_mode = "compiled"     # recurrence, compiled, eager
...
`recurrence` is the default. Eager and recurrence require a compatible prepared
`.pyamplicol-model` kernel pack; `built-in-sm` selects the packaged JIT O2 pack
automatically. Raw JSON/UFO model IR normally uses `compiled` unless you first
create a prepared bundle.
