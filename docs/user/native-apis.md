...
With `generation.emit_api_bundle = true` (the default), an artifact contains:
...
At non-f64 precision, the Python driver keeps decimal strings as `Decimal`
values without an intermediate f64 round trip. Native drivers convert the same
input to f64 and accept only precision 16.
...
metadata getters, parameter updates, warning access, total evaluation, and
resolved evaluation. Use `rusticol-config` rather than hard-coding include or
library paths:
...
`rusticol.hpp` is a header-only RAII wrapper over C ABI v1:
...
`type(rusticol_runtime)` owns load/close, metadata, parameter, warning, and
total/resolved f64 methods. Resolved Fortran storage is `(color, helicity,
point)`, the column-major view of the C ABI sequence `(point, helicity, color)`.

## Rust 2021

`rusticol.rs` is a dependency-free safe wrapper over C ABI v1. It owns the
handle, frees it on drop, exposes typed metadata and selectors, and remains
bound to its creating thread.
...
precision. See [Symbolica and Licensing](symbolica-and-licensing.md).
...
| Target incompatibility | Use a portable JIT artifact (compiled O1/O2, or eager/recurrence from a prepared JIT O2 pack), or regenerate C++/ASM/O0/O3 on the destination target. |
