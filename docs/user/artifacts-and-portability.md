...
| Prepared model bundle | `.pyamplicol-model` | Compiled model plus one prepared local-kernel backend for recurrence/eager generation. |
...
default **recurrence** and **eager** modes require a compatible prepared model
...
resulting process artifact. Eager and recurrence generation copy only the
needed kernels and compact schedules into the process artifact. A machine that
evaluates that artifact does not need the original `.pyamplicol-model` bundle.
