...
classes.  If \(\rho(f)\) represents the class containing \(f\), the recurrence
...
The original-\AC{} reference uses two different published routes.  For the
selected-flow workload, its generation value measures creation of the
table-driven process library.  For the all-flow workload, the reported setup
value measures preparation of direct iMode-2 evaluation.  The corresponding
runtime measurements implement the same physical workloads as \PAC{}, but the
all-flow setup quantities do not define a like-for-like process-generation
speedup.  The corresponding table therefore reports both quantities
absolutely and marks the \PAC{} entry as not comparable.
