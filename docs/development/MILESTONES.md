...
| Model independence hardening | Main + physics/model reviewers | In progress | Hand-written SM model, lowering, tensors, process catalog, selection, and validation are quarantined under `models.builtin`; shared process/color/DAG code consumes structural statistics, representation, anti-relation, source-orientation, spin/statistics, and exact quantum-flow metadata. Canonical typed `SourceIR`/`CrossingIR`, `PropagatorIR`, and model-owned `ContractionIR` records drive DAG construction, sources, propagation, root closure, stage evaluation, and Rust validation. Schema-v8 compilation emits explicit direct and closure contractions, keeps built-in SM forms exact, derives external forms only from proven structural metadata, and rejects untrusted compiler-owned dimensions or auxiliary roles. Runtime mass updates cannot invalidate the generated state space, and old artifacts without the typed contracts fail with an actionable regeneration error. Default versus model-supplied UFO propagators are classified from normalized expressions rather than object names. Runtime normalization and its mutable parameters are model-owned; generic schema construction no longer probes built-in coupling attributes or emits redundant compatibility aliases. Global Yang--Mills reductions require closure over every reachable compiled interaction. Model-compiler v8 serializes each eligible external four-point term's exact contact-decomposition proof or structured unsupported reasons, including color factors, dummy mapping, component basis/expansion, orientation parity, and scalar prefactors; the proof policy participates in compiled-model cache identity and is consumed without rediscovery. Generic color plans now serialize only physical sectors and admissible traversal words; dead legacy-order and inert Idenso state are removed. The complete 19-process external-SM topology/reuse ladder is invariant under unrelated signed-PDG labels and reordered UFO particle, vertex, coupling, color/Lorentz basis, and coupling-matrix inventories. Persisted tensor-order metadata and component-axis/dummy-index numerical invariance remain | Pending |
...
5. Complete the active model-independence hardening pass after freezing the
   expanded topology/performance baseline: add tensor-order metadata, then run
   relabeled-PDG and reordered-tensor invariance checks. Model-owned
   `ContractionIR`, serialized fail-closed colored-contact proofs, and
   reachable-interaction closure for global all-adjoint reductions are
   complete.
