...
5. Complete the active model-independence hardening pass after freezing the
   expanded topology/performance baseline: add explicit contact-decomposition
   and tensor-order metadata, then run relabeled-PDG and reordered-tensor
   invariance checks. Model-owned `ContractionIR`, fail-closed colored-contact
   handling, and reachable-interaction closure for global all-adjoint
