...
| Explicit tensor-ordering IR | Main + Goodall | Complete | Model-compiler/schema 9 records content-addressed axis, basis, storage, expansion, source-index, full/chiral embedding, projection, and kernel-link contracts; strict round-trip/corruption and reordered-source tests pass, along with built-in, external-SM, scalar-contact, scalar-gravity, Rusticol, and portable self-test checks | Pending |
...
5. Complete the active model-independence hardening pass after freezing the
   ...
   metadata, relabeled-PDG/source-inventory invariance, model-owned
   `ContractionIR`, serialized fail-closed colored-contact proofs, and
   reachable-interaction closure are complete; independent numerical
   component-axis and dummy-index permutation checks remain.
