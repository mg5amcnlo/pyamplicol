...
| Model independence hardening | Main + physics/model reviewers | In progress | Hand-written SM model, lowering, tensors, process catalog, selection, and validation are quarantined under `models.builtin`; shared process/color/DAG code consumes structural statistics, representation, anti-relation, source-orientation, and exact quantum-flow metadata. Canonical typed `SourceIR`/`CrossingIR`, `PropagatorIR`, and `ContractionIR` contracts drive DAG construction, sources, propagation, root closure, stage evaluation, and Rust validation. Runtime mass updates cannot invalidate the generated state space, and old schema-v3 artifacts without the typed contracts fail with an actionable regeneration error. Default versus model-supplied UFO propagators are classified from normalized expressions rather than object names. Global Yang--Mills reductions require closure over every reachable compiled interaction, and unsupported colored four-point contacts remain unlowered for structured preflight rejection. Generic color plans now serialize only physical sectors and admissible traversal words; dead legacy-order and inert Idenso state are removed. The validated slice preserves currents, interactions, roots, reduction groups, and resolved values across built-in, UFO/JSON SM, scalar, and scalar-gravity cases. Model-owned contraction production, explicit contact-decomposition proof records, tensor-order metadata, and relabeled/reordered adversarial fixtures remain | Pending |
...
5. Complete the active model-independence hardening pass after freezing the
   expanded topology/performance baseline: make `ContractionIR` production
   model-owned, add explicit contact-decomposition and tensor-order metadata,
   and run relabeled-PDG and reordered-tensor invariance checks. ContractionIR
   consumers, fail-closed colored-contact handling, and reachable-interaction
   closure for global all-adjoint reductions are complete.
