...
does not introduce recurrence/eager execution or a plane-aware evaluator ABI.
Later milestones include the explicitly authorized compact colour-contraction
representations, while retaining the same compiled execution mode.
...
## Milestone 15: deterministic SymJIT funclet compression and broader Walsh plans

Status: validated candidate milestone.

SymJIT compression is now a public, reproducible evaluator setting:
`evaluator.jit.compress = true` by default, with
`--jit-compress/--no-jit-compress` and card/provenance support. The setting is
forwarded through ordinary and prepared-model compilation. Legacy prepared
packs without the field are interpreted as uncompressed, matching how they
were produced. Compression changes generated machine code only; the
storage-v3 evaluator and pyAmpliCol artifact ABIs are unchanged.

Upstream SymJIT 2.21.1 already supported outlined funclets on x86. The
contributor build now applies one exact AArch64 parity patch to upstream
...
- Patch SHA-256:
  `da1add46715e39071518f326322985a75d5eb0fe9762e3a585f824117b45573f`.
- Fully configured source-tree SHA-256:
  `1f4db1ac71d1a4e80f80e9726ed4274bccd3e4666ea4d016b9acd3f52f23952d`.
- Contributor-lock SHA-256:
  `f8d1b82d58af9c76af9e57c29f3de1267ec15152c11e99fd428816099fc4e414`.

The installer verifies the patch revision, path, bytes, idempotent application,
and resulting tree, and records them in install state and reference
provenance. Patch bytes participate in all native-input digest
implementations. The strict release lane remains fail-closed: this is a
patched candidate dependency, not a claim that the required upstream release
has been published.

On the retained selected-flow Z+6g evaluator set, loaded machine code shrinks
from `12,077,800` to `9,574,120` bytes (`-20.73%`). Synthetic AArch64 scalar,
SIMD, and fast-complex kernels shrink by `51.2%`, `50.7%`, and `65.6%`
respectively. Deterministic funclet ordering makes repeated compile and
storage-v3 load/resave output byte-stable.

Seven-sample alternating native-wall measurements used identical artifacts
apart from the explicit compression setting, identical batches and selectors,
and caller-side packing outside the timed envelope:

| Workload | Batch | Uncompressed ms/point | Compressed ms/point | Change |
|---|---:|---:|---:|---:|
| Z+6g selected flow, helicity sum | 1 | 0.204705 | 0.177502 | -13.29% |
| Z+6g selected flow, helicity sum | 128 | 0.070078 | 0.049004 | -30.07% |
| Z+6g selected flow, helicity sum | 1024 | 0.073248 | 0.053627 | -26.79% |
| Z+6g all-flow union, fixed helicity | 128 | 0.398502 | 0.289308 | -27.40% |
| Z+6g all-flow union, fixed helicity | 1024 | 0.425390 | 0.323710 | -23.90% |
| `g g > t t~ + 3g`, NLC | 128 | 1.563022 | 1.081102 | -30.83% |
| `g g > t t~ + 3g`, NLC | 1024 | 1.789766 | 1.326480 | -25.89% |
| `g g > t t~ + 3g`, full | 128 | 1.479821 | 1.037285 | -29.90% |
| `g g > t t~ + 3g`, full | 1024 | 1.678007 | 1.280818 | -23.67% |

The fresh final selected-flow on/off artifacts were generated by candidate
`329d3c221948`; the generation pair completed in `10.70 s` total with
watchdog peaks `0.254 / 0.305 GiB`. Their artifact IDs are
`a6982ccde4d5a9a54837e5fafdfecf0eabd64a8ba8444d01887e9e22cd8ad874`
and
`f2d357286dabecdfa4d95b13c7279a07e4c83e5177f473e90e86bc66dac989f9`.
Earlier same-card JIT phases were `4.021 s` uncompressed and `3.691 s`
compressed, so the wall gain does not trade against generation time.

The exact K4/Walsh recognizer now also accepts structurally eligible NLC
matrices and complete multi-open-line word-set unions. It discovers
process-independent label-swap automorphisms, proves they are commuting free
involutions, checks the matrix action and every K4 block exactly, and falls
back for partial, duplicate, or asymmetric orbits. Existing complete
permutation-orbit plans remain byte-identical.

For `g g > t t~ + 3g` NLC, the plan reduces `1,500` upper Hermitian dots to
`1,108`; isolated against an otherwise identical compressed artifact this
improves native wall by `1.34%` at batch 128 and `2.28%` at batch 1024.
For `d d~ > t t~ + 3g`, the generic two-open-line recognition reduces NLC
`384 -> 232` dots and full `984 -> 312`. Recognition overhead on the
eight-coloured-leg `d d~ > t t~ + 4g` in-memory plans is `25.6 ms` NLC and
`58.3 ms` full, only about `1.4% / 3.1%` of colour-plan construction.

Numerical agreement is bitwise for selected batch 1 and the union/NLC
compression controls. The largest observed relative difference across the
remaining compression and NLC-Walsh comparisons is `2.50e-15`, safely inside
`rtol=1e-12`, `atol=1e-15`.

Evidence:

- Final selected-flow A/B:
  `/private/tmp/pyamplicol-symjit-compress-final/z6g-selected-ab.json`,
  SHA-256
  `e68238bad25e58903cd013e4a2138d7a89b2c693b478cc631c19ab060f4cc6e8`.
- Union, NLC compression, NLC Walsh, and full compression A/B SHA-256:
  `668d053160dfd46350c0d444b10afbe699e1192703fd17d5203ae0ebdb3735af`,
  `2bf1361ea460daa35d1263cf1f277b65e47f065255819d221b7e3bc536ea00b8`,
  `2a9b0797cd1d1a7bfd3a54562ea15d824da32f3260157cb28939585cced17479`,
  and
  `5196ed6a728ed9b894de3c538d9cd4642ff14283979e3b9d41698c92adc37cf7`.
- Focused candidate build peak: `1.306 GiB`; wheel SHA-256:
  `7d999241e20cc4ee008fdbb620c684b00448a4804d1ed8fa101043a9101b0754`.
- Candidate dependency gate: ready with no issues. Focused dependency,
  configuration, colour, prepared-model, and generation tests:
  `148 passed`; one packaged eager-asset test remains blocked solely by the
  already documented stale candidate asset fingerprint. Ruff and
  `git diff --check` pass.

A true variable-rank `C2^k` artifact/runtime representation is deferred to a
separate milestone. For `g g > t t~ + 4g`, rank-three H8 would reduce NLC
`18,720 -> 13,032` dots and full roughly `259,560 -> 32,736`, compared with
