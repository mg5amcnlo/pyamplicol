...
`4e288ce5f3132b05e2a81eb6452c011b9e2bb936`. Contributor and release
...
It also sets the currently latent direct-arena operation to overwrite and
identity-output scaling to true, so persisted semantics cannot change if
SymJIT later consumes those options. Arena mode selects SymJIT's standard
Sympy-style P-kernel:

```text
f(null, plane_descriptors, point_or_block_index, params)
```

The application retains the ordinary
`symjit-application-storage-v3` portable-storage contract. pyAmpliCol records
a separate `pyamplicol-symjit-plane-application-v1` binding contract for
...
SIMD indices are block indices. Rusticol handles unaligned heads and tails
with the scalar P-kernel and uses the available two-lane AArch64 or four-lane
x86 kernel only for complete blocks. A scalar-only kernel remains a supported
fallback.
...
There is no compatibility loader which rewrites those executable formats.
