...
\end{equation}
External momenta are nevertheless supplied in the physical convention of
Eq.~\eqref{eq:zgg-physical-process}; crossing is applied internally.
...
configurations.  Twenty-four vanish identically for the chiral source and
kernel conventions; they remain explicit zero entries in resolved results.
...
The two leading-colour layouts described in
Sec.~\ref{sec:lc-flow-layouts} evaluate the same complete set of helicity and
flow components.  One is optimized for a selected flow with summed helicities;
...
For the built-in compatibility model, the normalization used in this example
is
\begin{equation}
  |{\cal M}|^2
