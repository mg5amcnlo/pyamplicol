...
impl<'a> RecurrenceProcessInputView<'a> {
    ...
    fn validate_public_flows(self, catalogs: &ProcessCatalogs<'_>) -> RusticolResult<()> {
        ...
        for (flow_index, flow) in self.public_lc_flows.iter().copied().enumerate() {
            ...
            let legacy_two_line_word = self.amplicol_legacy_two_line_public_word(
                construction_sector,
                construction_word,
                permutation,
            )?;
            if !reflected_trace && legacy_two_line_word.as_deref() != Some(public_word) {
    ...
    fn amplicol_legacy_two_line_public_word(
        self,
        sector: ProcessPhysicalLCSectorRow,
        construction_word: &[u32],
        permutation: &[u32],
    ) -> RusticolResult<Option<Vec<u32>>> {
        if sector.kind()? != ProcessLCSectorKind::OpenLines || sector.open_string_range.count != 2 {
            return Ok(None);
        }
        let lines = &self.lc_open_strings[sector.open_string_range.as_usize_range(
            self.lc_open_strings.len(),
            "legacy public-flow open strings",
        )?];
        for line in lines {
            if !self
                .sequence(line.adjoint_sequence_id, "legacy public-flow adjoints")?
                .is_empty()
                || !self
                    .sequence(line.singlet_sequence_id, "legacy public-flow singlets")?
                    .is_empty()
            {
                return Ok(None);
            }
        ...
        let is_initial = |slot: u32| -> RusticolResult<bool> {
            Ok(self
                .external_legs
                .get(required_reference(
                    slot,
                    self.external_legs.len(),
                    "legacy public-flow endpoint",
                )?)
                .ok_or_else(|| invalid("validated legacy public-flow endpoint disappeared"))?
                .is_initial
                != 0)
        };
        let mut initial_to_final = None;
        let mut final_to_initial = None;
        for line in lines {
            match (
                is_initial(line.fundamental_source_slot)?,
                is_initial(line.antifundamental_source_slot)?,
            ) {
                (true, false) if initial_to_final.is_none() => {
                    initial_to_final = Some(*line);
                }
                (false, true) if final_to_initial.is_none() => {
                    final_to_initial = Some(*line);
                }
                _ => return Ok(None),
            }
        }
        let (Some(incoming), Some(outgoing)) = (initial_to_final, final_to_initial) else {
            return Ok(None);
        };
        let expected_construction = [
            incoming.fundamental_source_slot,
            incoming.antifundamental_source_slot,
            outgoing.fundamental_source_slot,
            outgoing.antifundamental_source_slot,
        ];
        if construction_word != expected_construction {
            return Ok(None);
        }
        let map = |slot: u32| -> RusticolResult<u32> {
            permutation
                .get(required_reference(
                    slot,
                    permutation.len(),
                    "legacy public-flow source permutation",
                )?)
                .copied()
                .ok_or_else(|| invalid("validated legacy public-flow permutation disappeared"))
        };
        Ok(Some(vec![
            map(outgoing.fundamental_source_slot)?,
            map(outgoing.antifundamental_source_slot)?,
            map(incoming.fundamental_source_slot)?,
            map(incoming.antifundamental_source_slot)?,
        ]))
    }
...
mod tests {
    ...
    #[test]
    fn crossed_two_line_sector_authenticates_legacy_public_word() {
        let external_legs = [
            ProcessExternalLegRow {
                source_slot: 0,
                public_label: 1,
        ...
        let sequence_ranges = [test_range(0, 0), test_range(0, 4)];
        let sequence_values = [1, 3, 2, 0];
        let input = RecurrenceProcessInputView {
            input_abi: RECURRENCE_BUILDER_INPUT_ABI,
            declared_input_digest: SemanticDigest::new(test_digest(41))
                .expect("test process digest is nonzero"),
            fermion_pairing: None,
            bitset_ranges: &[],
            bitset_words: &[],
        ...
        let sector = ProcessPhysicalLCSectorRow {
            sector_id: 1,
            public_id_string_id: 0,
            ...
            open_string_range: test_range(0, 2),
            trace_sequence_id: 0,
            singlet_sequence_id: 0,
            word_sequence_id: 1,
            support_mask_id: 0,
        ...
        assert_eq!(
            input
                .amplicol_legacy_two_line_public_word(sector, &sequence_values, &[0, 1, 2, 3],)
                .unwrap(),
            Some(vec![2, 0, 1, 3])
        );
    }
