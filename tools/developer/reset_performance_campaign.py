...
def _verify_baseline(arguments: argparse.Namespace) -> dict[str, object]:
    ...
    if (
        ...
        raise CampaignResetError("baseline gate does not cover exactly 1,796 cells")
    amplicol = {
        cell.cell_id
        for cell in cells
    ...
    if (
        ...
        raise CampaignResetError(
            "baseline gate catalog split differs from 314/1,482"
        )
    ...
    if page_values != ["73"]:
        raise CampaignResetError("baseline PDF does not contain exactly 73 pages")
