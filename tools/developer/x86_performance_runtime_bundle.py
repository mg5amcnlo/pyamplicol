...
FROZEN_SOURCE_ROOT = Path("/private/tmp/pyamplicol-eager-compiled-arena-base-src")
...
def frozen_cargo_config_bytes() -> bytes:
    root = FROZEN_SOURCE_ROOT.as_posix()
    return (
        "# Generated by dependencies/install_dependencies.py\n"
        "[patch.crates-io]\n"
        f'graphica = {{ path = "{root}/dependencies/checkouts/'
        'symbolica/lib/graphica" }\n'
        f'numerica = {{ path = "{root}/dependencies/checkouts/'
        'symbolica/lib/numerica" }\n'
        f'symbolica = {{ path = "{root}/dependencies/checkouts/symbolica" }}\n'
        f'symjit = {{ path = "{root}/dependencies/checkouts/symjit" }}\n'
    ).encode()
...
def _baseline_ignored_inventory(root: Path) -> dict[str, object]:
    ...
    ignored_entries_by_bootstrap_root = {
        relative.rstrip("/"): sum(path.startswith(relative) for path in ignored)
        for relative in _BASELINE_BOOTSTRAP_ROOTS
    }
    return {
...
def freeze_baseline(source_root: Path) -> dict[str, object]:
    ...
    if hashlib.sha256(frozen_cargo_config_bytes()).hexdigest() != (
        FROZEN_CARGO_CONFIG_SHA256
    ):
        raise BundleError("frozen cargo-config path contract is inconsistent")
...
def _valid_ignored_relative_path(value: object) -> bool:
    if not isinstance(value, str) or not value or "\\" in value:
        return False
    directory_marker = value.endswith("/")
    path = value[:-1] if directory_marker else value
    if not path:
        return False
    normalized = PurePosixPath(path)
    expected = normalized.as_posix() + ("/" if directory_marker else "")
    return (
        value == expected
        and expected not in {".", "./"}
...
def _valid_baseline_attestation_payload(payload: Mapping[str, object]) -> bool:
    ...
    if (
        ...
        return False
    ignored_paths = source_inventory.get("ignored_relative_paths")
    ignored_counts = source_inventory.get("ignored_entries_by_bootstrap_root")
    bootstrap_paths = {
        relative.rstrip("/"): relative for relative in _BASELINE_BOOTSTRAP_ROOTS
    }
    if (
        not isinstance(ignored_paths, list)
        or not 0 < len(ignored_paths) <= _BASELINE_IGNORED_ENTRY_LIMIT
        or any(not _valid_ignored_relative_path(path) for path in ignored_paths)
        or ignored_paths != sorted(set(ignored_paths))
        or source_inventory.get("ignored_entry_count") != len(ignored_paths)
        or source_inventory.get("ignored_paths_sha256")
        != _canonical_sha256(ignored_paths)
        or not isinstance(ignored_counts, Mapping)
        or set(ignored_counts) != set(bootstrap_paths)
        or any(not _nonnegative_integer(count) for count in ignored_counts.values())
        or any(relative not in ignored_paths for relative in _BASELINE_GENERATED_FILES)
        or any(
            path not in _BASELINE_GENERATED_FILES
            and not any(path.startswith(prefix) for prefix in bootstrap_paths.values())
            for path in ignored_paths
        )
        or any(
            ignored_counts[logical]
            != sum(path.startswith(prefix) for path in ignored_paths)
            for logical, prefix in bootstrap_paths.items()
        )
        or len(_BASELINE_GENERATED_FILES) + sum(ignored_counts.values())
        != len(ignored_paths)
    ):
        return False
    ...
    return (
        isinstance(generated_files, Mapping)
        and set(generated_files) == set(_BASELINE_GENERATED_FILES)
        and all(
        ...
        and isinstance(bootstrap_trees, Mapping)
        and set(bootstrap_trees) == expected_bootstrap_paths
        and all(
