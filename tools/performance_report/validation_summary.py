...
def _coverage_status(passed: int, expected: int) -> str:
    if passed == expected:
        return r"\textcolor{ReportGreen}{verified}"
    return rf"\textcolor{{ReportOrange}}{{incomplete ({expected - passed} open)}}"
    ...
    *,
    ...
    for n_final, expected in summary.expected_by_n:
        ...
        lines.append(
            ...
            f"{_coverage_status(passed, expected)}"
            r" \\"
        )
    lines.extend(
        [
            ...
            (
                ...
                f"{_coverage_status(summary.passed_total, summary.expected_total)}"
                r" \\"
            ),
            r"\bottomrule",
            r"\end{tabular}",
