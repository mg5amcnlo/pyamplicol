...
def _rusticol_config() -> str | None:
    ...
    if sibling.is_file():
        return str(sibling)
    return shutil.which("rusticol-config")
    ...
    environment.update(
        {
            "CXX": tools.cxx,
            "FC": tools.fortran,
            "RUSTICOL_CONFIG": tools.rusticol_config,
        }
    )
    for language in ("cpp", "fortran"):
        completed = subprocess.run(
            [tools.make, "-C", str(artifact / "API" / language), "all"],
            env=environment,
            check=False,
            capture_output=True,
            text=True,
...
def _all_languages(
    ...
    *,
    ...
    return {
        ...
        for language in ("python", "cpp", "fortran")
    }
