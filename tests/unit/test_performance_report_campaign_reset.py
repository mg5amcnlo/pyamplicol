...
def test_reset_cardinality_matches_the_current_catalog() -> None:
    ...
    assert EXPECTED_CATALOG_CELL_COUNT == len(cells) == 2198
    assert EXPECTED_AMPLICOL_CELL_COUNT == amplicol == 314
    assert EXPECTED_NON_AMPLICOL_CELL_COUNT == len(cells) - amplicol == 1884
    assert (
        sum(REPORT_CATALOG.static_na_reason(cell) is not None for cell in cells) == 34
    )
    assert sum(REPORT_CATALOG.static_na_reason(cell) is None for cell in cells) == 2164
